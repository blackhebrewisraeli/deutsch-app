...
| 6   | Apply `supabase/migrations/20260918200000_revoke_is_league_member_execute.sql` to Sprachschule after that PR merges. Dashboard SQL editor. Never `migration repair`. Revokes `EXECUTE` on `is_league_member` from `anon` / `authenticated` (advisor 0029). Then confirm Stats → Ligen still loads for a signed-in user. | **Unapplied** — owner-only after return. Procedure: `docs/PRE_BETA_OWNER_CHECKLIST.md` §8 |
