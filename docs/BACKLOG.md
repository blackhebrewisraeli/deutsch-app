...
| 10  | Add `com.sprachschule.deutsch://login-callback` to the hosted **URL Configuration → Redirect URLs** (keep every existing entry, leave the Site URL alone). Without it Supabase silently sends every sign-in started in the iOS / Android app to the website instead of back to the app. | **Not done** — owner-only; blocks native sign-in. Procedure and on-device checks: `docs/MOBILE_AUTH_SETUP.md` |
