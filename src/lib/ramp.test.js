...
describe('deriveSurfaceRamp', () => {
  it('keeps legacy surface / surface-alt values for light', () => {
    const p = MODE_COLORS.light;
  ...
  it('keeps legacy surface / surface-alt values for dark', () => {
    const p = MODE_COLORS.dark;
  ...
  // Regression: light shipped surface-2 #FFFFFF → surface-3 #FEFEFE, one
  ...
  it('gives every palette a top elevation step that is actually visible', () => {
    for (const mode of ['light', 'dark']) {
      const ramp = deriveSurfaceRamp(MODE_COLORS[mode], mode);
      expect(
        contrastRatio(ramp['surface-2'], ramp['surface-3']),
        `${mode} surface-2 → surface-3`
  ...
  it('keeps every consecutive elevation pair distinguishable', () => {
    for (const mode of ['light', 'dark']) {
      const r = deriveSurfaceRamp(MODE_COLORS[mode], mode);
  ...
  // fixed grey, so a warm palette never tips cool. Light's `surface` is pure
  // white, so the 5% shade toward `fg` rounds to an exactly neutral byte value
  // (#F3F3F3) rather than a visibly warm one — >= still catches a regression
  // that skews the step blue (cool) without asserting a strict inequality
  // 8-bit rounding cannot guarantee.
  it('shades light-mode surface-3 within the palette rather than to a cool grey', () => {
    const r = deriveSurfaceRamp(MODE_COLORS.light, 'light');
    const [red, , blue] = [1, 3, 5].map((i) => parseInt(r['surface-3'].slice(i, i + 2), 16));
    expect(
      red,
      'never skews cool: red must not fall below blue in surface-3'
  ...
  it('lifts surface-3 only as far as every body ink stays at AA', () => {
    ...
    for (const mode of ['light', 'dark']) {
      const p = MODE_COLORS[mode];
      const s3 = deriveSurfaceRamp(p, mode)['surface-3'];
      for (const ink of ['fg', 'fg-muted', 'fg-subtle']) {
        expect(contrastRatio(p[ink], s3), `${mode} ${ink} on surface-3`).toBeGreaterThanOrEqual(
          4.5
        );
      }
    }
  ...
  it('includes surface, accent, and semantic steps for every palette', () => {
    for (const mode of ['light', 'dark']) {
      ...
      for (const key of [
        ...
        expect(tokens[key], `${mode} ${key}`).toMatch(/^#[0-9A-Fa-f]{6}$/);
      }
    }
  });
});
